...
GibsSampler <- function(n,k){
  ...
  for (i in 2:n) {
    muold <- unlist(initx[i-1, ])
    munew <- vector()
    munew[1] <- rnorm(1,(muold[2]+Y[1])/2,0.2/2)
    for (j in 2:(k-1)) {
      munew[j] <- rnorm(1,(muold[j-1]+muold[j+1]+Y[j])/3,0.2/3)
    }
    munew[k] <- rnorm(1,(muold[k-1]+Y[k])/2,0.2/2)
    initx <- rbind(initx,munew)
    muold <- munew
  }
  return(initx)
}
