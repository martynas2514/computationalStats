...
$$Q(\lambda, \lambda^k)= ln(\sum x_i^2)- nln(2)-2nln(\lambda)- \frac {\sum X_i Y_i}{\lambda} - \frac{\sum^{obs} X_i Z_i}{2 \lambda} - \frac {j \lambda^k}{\lambda} $$
Now we should derive this function according to $\lambda$ and set to zero.
$$ \frac{\partial{E} [L(\lambda)] }{\partial{\lambda}} = -\dfrac{4n \lambda-\sum^{n} X_i Y_i-2\sum^{obs} X_i Z_i -2j\lambda^k}{2\lambda^2}$$
$$\lambda = \dfrac{\sum^{obs} X_i Z_i+2\sum^{n} X_i Y_i+2j\lambda^k}{4n}$$
