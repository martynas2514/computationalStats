...
For this question we are required to make a scatter plot and conclude wether the lottery seems random. 
...
An estimate $\hat{Y}$ will be computed using `r #loess()` and afterwards plot $\hat{Y}(X)$ adding it to the previous plot.
...
From estimates we can see that there is some pattern in the data, however it's hardly visible.
